/**
 * 图像/PDF 处理器页面组件
 * 功能：支持图片格式转换、PDF 转图片、图片压缩等
 * 特点：所有操作均在浏览器本地完成，不上传服务器，保护隐私
 */
import { useCallback } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { useImageTool } from '@/hooks/useImageTool' // 自定义 Hook，封装了核心逻辑
...
export default function ImageProcessor() {
   ...
   * 从自定义 Hook 中获取状态和方法
   * items: 当前文件队列
   * options: 转换配置（格式、质量、缩放等）
   * isProcessing: 是否正在处理中
   * processAll: 开始处理全部文件
   * downloadAll: 下载所有处理完成的文件
   */
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      addFiles(e.dataTransfer.files) // 将拖入的文件添加到队列
    }
  }, [addFiles])
